...
export const FinalSummary: React.FC<FinalSummaryProps> = ({
  ...
  return (
    <div className="relative w-full h-full min-h-[580px] flex flex-col justify-between p-4 sm:p-6 lg:p-8 bg-[#040608] text-white select-none overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-4 pb-4 border-b border-white/[0.12]">
        {/* Project Branding */}
        <div className="flex items-center gap-4 sm:gap-6">
          <div className="relative h-14 sm:h-16 w-44 sm:w-56 flex-shrink-0">
            <Image
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-[#c5a869] animate-pulse" />
              <span className="text-sm sm:text-base font-mono tracking-[0.25em] text-[#c5a869] uppercase font-bold">
                Live Availability Matrix
              </span>
            </div>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-light tracking-wide text-white font-sans mt-0.5">
        ...
        <div className="flex items-center gap-5 sm:gap-7 lg:gap-10 text-base font-mono tracking-wider flex-wrap">
          <div className="flex items-baseline gap-3">
            <span className="text-white/60 uppercase text-xs sm:text-sm font-sans font-semibold">Total</span>
            <span className="text-3xl sm:text-4xl md:text-5xl font-light text-white font-mono">
              <AnimatedCounter value={overallStats.total} />
            </span>
          </div>
          <span className="text-white/20">/</span>
          <div className="flex items-baseline gap-3">
            <span className="text-[#c5a869] uppercase text-xs sm:text-sm font-sans font-semibold">Available</span>
            <span className="text-3xl sm:text-4xl md:text-5xl font-light text-[#c5a869] font-mono">
              <AnimatedCounter value={overallStats.available} />
            </span>
          </div>
          <span className="text-white/20">/</span>
          <div className="flex items-baseline gap-3">
            <span className="text-[#be185d] uppercase text-xs sm:text-sm font-sans font-semibold">Booked</span>
            <span className="text-3xl sm:text-4xl md:text-5xl font-light text-[#be185d] font-mono">
              <AnimatedCounter value={overallStats.booked} />
            </span>
          </div>
          <span className="text-white/20">/</span>
          <div className="flex items-baseline gap-3">
            <span className="text-[#c58a2f] uppercase text-xs sm:text-sm font-sans font-semibold">Blocked</span>
            <span className="text-3xl sm:text-4xl md:text-5xl font-light text-[#c58a2f] font-mono">
              <AnimatedCounter value={overallStats.blocked} />
            </span>
          </div>
          <span className="text-white/20">/</span>
          <div className="flex items-baseline gap-3">
            <span className="text-[#ea580c] uppercase text-xs sm:text-sm font-sans font-semibold">LO-Blocked</span>
            <span className="text-3xl sm:text-4xl md:text-5xl font-light text-[#ea580c] font-mono">
              <AnimatedCounter value={overallStats.loBlocked} />
            </span>
          </div>
          <span className="text-white/20">/</span>
          <div className="flex items-baseline gap-3">
            <span className="text-[#94a3b8] uppercase text-xs sm:text-sm font-sans font-semibold">NFS</span>
            <span className="text-3xl sm:text-4xl md:text-5xl font-light text-[#94a3b8] font-mono">
      ...
      <div className="flex-1 flex flex-col justify-center my-auto py-3">
        ...
        <div className="text-center mb-6 sm:mb-8">
          <span className="text-sm sm:text-base md:text-lg font-mono tracking-[0.35em] text-white/60 uppercase font-bold">
        ...
        <div className="relative w-full h-[280px] sm:h-[320px] lg:h-[360px] flex">
          ...
          {/* Y-Axis Scale */}
          <div className="relative h-full flex flex-col justify-between pr-4 py-0 text-right select-none" style={{ minWidth: '42px' }}>
            {yTicks.slice().reverse().map(tick => (
              <span key={tick} className="text-sm sm:text-base font-mono text-white/50 leading-none font-bold">
                {tick}
              </span>
            ))}
          </div>
          ...
          <div className="relative flex-1 h-full border-l border-b border-white/[0.15]">
            
            {/* Horizontal Gridlines (Ultra-Subtle) */}
            {yTicks.map(tick => (
              <div
                key={tick}
                className="absolute w-full border-t border-white/[0.05] pointer-events-none"
              ...
              {floorData.map(fd => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span
                        className={`text-sm sm:text-base md:text-lg font-mono font-extrabold transition-colors duration-200 ${
                          isHovered ? 'text-[#D4AF37]' : 'text-white/80'
                        }`}
                      >
                        {fd.total}
                      </span>
                    ...
                    <div
                      className={`w-full max-w-[32px] sm:max-w-[36px] md:max-w-[42px] rounded-t-[6px] overflow-hidden flex flex-col transition-all duration-200 border backdrop-blur-md relative ${
                        isHovered
                          ? 'border-[#c5a869] shadow-[0_0_24px_rgba(197,168,105,0.45)] ring-2 ring-[#c5a869]/60 bg-white/[0.08]'
                          : 'border-white/25 bg-white/[0.04] shadow-[0_4px_16px_rgba(0,0,0,0.6)]'
                      }`}
                      style={{
                        height: `${barHeightPct}%`,
                      }}
                    >
                      ...
                      <div className="absolute inset-y-0 left-0 w-1/3 bg-gradient-to-r from-white/[0.12] to-transparent pointer-events-none z-10" />

                      {/* Segments Stacked Inside as Translucent Architectural Layers */}
                      {SEGMENTS.slice().reverse().map(seg => {
                        ...
                        return (
                          <div
                            key={seg.key}
                            className="w-full relative flex items-center justify-center transition-all duration-150 border-b border-black/50 first:border-b-0"
                            style={{
                          ...
                          >
                            ...
                            {segmentHeightPct >= 18 && (
                              <span className="text-xs sm:text-sm md:text-base font-mono font-extrabold text-white drop-shadow-[0_1px_4px_rgba(0,0,0,1)] relative z-20">
                                {count}
                              </span>
                            )}
                          </div>
                    ...
                    {isHovered && (
                      ...
                      >
                        <div className="bg-[#080c14]/98 border-2 border-white/25 rounded-2xl px-6 py-4 shadow-[0_20px_50px_rgba(0,0,0,0.98)] backdrop-blur-2xl min-w-[220px]">
                          <div className="text-sm tracking-[0.2em] text-[#c5a869] uppercase font-mono font-bold mb-2.5">
                            {FLOOR_TERMINOLOGY[fd.floor]?.display || `Floor ${fd.floor}`}
                          </div>
                          <div className="h-[1px] bg-white/[0.15] mb-3" />
                          
                          {SEGMENTS.map(seg => {
                            const val = fd[seg.key];
                            return (
                              <div key={seg.key} className="flex items-center justify-between gap-6 py-1">
                                <div className="flex items-center gap-3">
                                  ...
                                  />
                                  <span className="text-sm sm:text-base font-sans text-white/90 font-medium">{seg.label}</span>
                                </div>
                          ...
                          <div className="h-[1px] bg-white/[0.15] mt-3 mb-2.5" />
                          <div className="flex items-center justify-between pt-0.5">
                            <span className="text-sm text-white/60 uppercase font-sans tracking-wider font-semibold">Total</span>
                            <span className="text-base sm:text-lg font-mono font-extrabold text-[#c5a869]">{fd.total}</span>
                          </div>
                        </div>
                        {/* Tooltip Downward Caret */}
                        <div className="w-3.5 h-3.5 bg-[#080c14] border-b-2 border-r-2 border-white/25 rotate-45 mx-auto -mt-2" />
                      </div>
                    )}
                  </div>
                );
            ...
            <div className="absolute -bottom-10 left-0 right-0 flex justify-between px-2 sm:px-4 md:px-6 gap-2 sm:gap-3 md:gap-4">
              {floorData.map(fd => (
                <div
                  key={fd.floor}
                  className={`flex-1 text-center text-sm sm:text-base md:text-lg font-mono font-extrabold tracking-wider transition-colors duration-200 cursor-pointer ${
        ...
        <div className="flex items-center justify-center gap-8 sm:gap-14 mt-16 flex-wrap">
          {SEGMENTS.map(seg => (
            <div key={seg.key} className="flex items-center gap-4">
              <span
                className={`w-5 h-5 rounded-[5px] flex-shrink-0 shadow-sm border ${seg.legendBorder}`}
                style={{ backgroundColor: seg.legendFill }}
              />
              <span className="text-sm sm:text-base md:text-lg font-sans text-white font-medium tracking-wide">
                {seg.label}
              </span>
            </div>
          ))}
      ...
      <div className="flex items-center justify-between text-xs sm:text-sm md:text-base text-white/50 tracking-[0.25em] uppercase border-t border-white/[0.12] pt-3 font-mono font-medium">
